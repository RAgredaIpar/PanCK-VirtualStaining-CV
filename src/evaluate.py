...
# --- 2. DATA PIPELINE (is_train=False desactiva el Data Augmentation) ---
...
if os.path.exists(M3_PATH):
    # try-except por si los nombres de las capas internas del .pth oficial varían un poco
    try:
...
with torch.no_grad():
    for i, (he, real_ihc, _) in enumerate(dataloader):
        ...
        if i == 0:
            ...
            # Rompemos el ciclo en la muestra 50 para no hacer la evaluación infinita
        if i > 50:
            break
